...
float Error_Rate_Fct(char * final_data, char * initial_data){
    ...
    final_data = strcat("/home/pi/Desktop/",final_data);
...
float loss_rate_Fct(char * final_data, char * initial_data){
    ...
    if (nb_lignes_i > nb_lignes_j) {
        ...
        float nb_data = nb_lignes_i * 5 ; // 8 parce que c'est un char

        int nb_donnees_j = sizeof(data_j[-1]);   //ici pareil à vérifier
        float nb_loss = nb_loss + (5-nb_donnees_j);   //en gros on vérifie que la dernière soit bien composée de 5 données sinon on ajoute
    }

    else {
        float nb_loss = nb_loss + (nb_lignes_j - nb_lignes_i);
        float nb_data = nb_lignes_j * 5 ;

        int nb_donnees_i = sizeof(data_i[-1]);   //ici pareil à vérifier
        float nb_loss = nb_loss + (5-nb_donnees_i);   //en gros on vérifie que la dernière soit bien composée de 5 données sinon on ajoute
...
int main(){
    char * initial_data = "Numerical_Results_capteur.txt";
    char * final_data = "test.txt";
